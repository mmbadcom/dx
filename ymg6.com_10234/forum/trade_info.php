...
<div class="ptm act cl"{if $_G[inajax]} style="border:0"{/if}>
	<div class="cl bus_trade">
	
		<div class="spvimg">
		<!--{if $trade['displayorder'] > 0}--><em class="hot">{lang post_trade_sticklist}</em><!--{/if}-->
		<!--{if $trade['thumb']}-->
			<a href="javascript:;"><img src="$trade[thumb]" onclick="zoom(this, '$trade[attachurl]')" width="{if $trade[width] > 450}450{else}$trade[width]{/if}" _width="450" _height="450" alt="$trade[subject]" /></a>
		<!--{else}-->
		...
		<dl class="nums">
			<dt class="pt10">{lang trade_price}:</dt>
			<dd>
				<!--{if $trade[price] > 0}-->
			...
			<!--{if $trade[price] && $trade['costprice'] > 0 || $_G['setting']['creditstransextra'][5] != -1 && $trade[credit] && $trade['costcredit'] > 0}-->
				<dt class="pt5">{lang trade_costprice}:</dt>
				<dd class="xg1">
		...
		<!--{if $post['authorid'] != $_G['uid'] && empty($thread['closed']) && $trade[expiration] > -1}-->
			<p class="pns mbm">
			<!--{if $trade[amount]}-->
				<button onclick="{if $_G['uid']}window.open('forum.php?mod=trade&tid=$post[tid]&pid=$post[pid]','','');{else}showWindow('login', 'member.php?mod=logging&action=login&guestmessage=yes');{/if}" class="bus_trade_btn"><span>{lang attachment_buy}</span></button> &nbsp;
			<!--{else}-->
				<button disabled="yes" class="pn"><span>{lang sold_out}</span></button> &nbsp;
			<!--{/if}-->
			<!--{if $_G['uid']}--><button onclick="showWindow('sendpm', 'home.php?mod=spacecp&ac=pm&op=showmsg&handlekey=showmsg_$post[authorid]&touid=$post[authorid]&pmid=0&daterange=2&tradeid=$post[pid]')" class="bus_trade_btn02"><span><!--{if $online}-->{lang on_line}<!--{/if}-->{lang trade_bargain}</span></button><!--{/if}-->
			</p>
		<!--{/if}-->
...
<div class="clear"></div>
  <div class="bus_box">
	<div class="cl mtw bus_trade_info">
		<div class="c pbm">
			$post[message]
		</div>
		<!--{if $post['attachment']}-->
...
</div><!-- Mobanbus_cn END bus_tradetop -->
