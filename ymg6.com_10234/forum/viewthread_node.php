...
<tr>
		...
		<div id="favatar$post[pid]" class="pls favatar">
			...
			<!--{if $post['authorid'] && $post['username'] && !$post['anonymous']}-->
				<!--{if $_G['setting']['authoronleft']}-->
					<div class="pi">
						<div class="authi"><a href="home.php?mod=space&uid=$post['authorid']" target="_blank" class="xw1"{if $post[groupcolor]} style="color: $post[groupcolor]"{/if}>$post[author]</a>$authorverifys</b></div>
					</div>
				<!--{/if}-->
	...
	<td class="plc"<!--{if $close_leftinfo}--> style="width:100%"<!--{/if}-->>
		<div class="pi"<!--{if $close_leftinfo && !$post['anonymous'] && $postshowavatars && $showavatars}--> style="height:30px"<!--{/if}-->>
			<!--{if !IS_ROBOT}-->
