...
<div id="ct" class="wp cl">
    <div class="bus_viewthread "> 
    	<div class="bus_yt" <!--{if $_G[forum][banner] && !$subforumonly}--> style=" background:url(data/attachment/common/$_G['forum'][banner])  top right no-repeat"<!--{/if}-->>
  ...
  <div class="bus_viewthread_top">
	...
	<!--{loop $postlist $post}-->
		...
		<div id="post_$post[pid]" {if $_G['blockedpids'] && $post['inblacklist']}style="display:none;"{/if}>
		  <div class="bus_viewthread_br">
			<!--{subtemplate forum/viewthread_node}-->
		  </div>
		</div>
		<!--{eval $postcount++;}-->
	<!--{/loop}-->
	<div id="postlistreply" class="pl"><div id="post_new" class="viewthread_table bus_viewthread_br" style="display: none"></div></div>
	<!--{if $_G['blockedpids']}-->
